...
function App() {
  ...
  // Resolve static images uploaded in Admin panel
  const coverImageObj = portfolioItems.find(item => item.type === 'cover_image');
  const coverImageUrl = coverImageObj ? coverImageObj.src : coverImage;
  ...
  const sangeetBgObj = portfolioItems.find(item => item.type === 'sangeet_bg');
  const sangeetBgUrl = sangeetBgObj ? sangeetBgObj.src : sangetImage;

  const haldiBgObj = portfolioItems.find(item => item.type === 'haldi_bg');
  const haldiBgUrl = haldiBgObj ? haldiBgObj.src : coverImage;

  const kidsBgObj = portfolioItems.find(item => item.type === 'kids_bg');
  const kidsBgUrl = kidsBgObj ? kidsBgObj.src : kidsImage;

  const corporateBgObj = portfolioItems.find(item => item.type === 'corporate_bg');
  const corporateBgUrl = corporateBgObj ? corporateBgObj.src : corporateImage;

  const culturalBgObj = portfolioItems.find(item => item.type === 'cultural_bg');
  const culturalBgUrl = culturalBgObj ? culturalBgObj.src : cultureImage;

  const fashionBgObj = portfolioItems.find(item => item.type === 'fashion_bg');
  const fashionBgUrl = fashionBgObj ? fashionBgObj.src : fashionImage;
  ...
  const filteredItems = portfolioItems.filter(item => {
    if (item.type !== 'photo' && item.type !== 'video') return false;
    return item.tag === viewingTag;
  });
  ...
  return (
    ...
    >
      ...
      <main>
        {viewingTag ? (
          /* Tag Detail / Gallery Page */
          <section className="py-24 md:py-32 px-6 md:px-8 bg-surface min-h-[75vh]" id="portfolio-gallery">
            <div className="max-w-7xl mx-auto">
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16 border-b border-outline-variant/10 pb-8">
                <div>
                  <button 
                    onClick={() => { setViewingTag(null); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                    className="flex items-center gap-2 text-primary hover:text-[#f2ca50] transition-colors mb-4 font-serif text-sm uppercase tracking-widest font-semibold"
                  >
                  ...
                  <p className="text-on-surface-variant font-headline text-lg md:text-xl italic mt-2">
                    {services.find(s => s.title === viewingTag)?.description}
                  </p>
                </div>
              </div>

              {filteredItems.length > 0 ? (
                /* Asymmetrical columns masonry grid */
                <div className="columns-1 sm:columns-2 lg:columns-3 gap-8 space-y-8 animate-[fadeInUp_0.4s_ease-out]">
                ...
                <div className="text-center py-20 bg-surface-container-low rounded-2xl border border-outline-variant/10">
                  <span className="material-symbols-outlined text-5xl text-outline mb-4">image_not_supported</span>
                  <p className="text-gray-500 italic">No photos or videos uploaded for {viewingTag} yet.</p>
                </div>
              )}
            </div>
          </section>
          ...
          <>
            ...
            {/*                       HERO SECTION                        */}
            {/* ========================================================= */}
            <section className="relative min-h-screen flex flex-col justify-end items-center overflow-hidden pb-24 md:pb-20" id="home">
              ...
              <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
                  {services.map((service, index) => (
                    <div 
                      key={index} 
                      onClick={() => { setViewingTag(service.title); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                      className="group relative bg-surface-container-low p-4 sm:p-6 md:p-10 h-[240px] sm:h-[320px] md:h-[400px] flex flex-col justify-end overflow-hidden rounded-xl transition-all duration-500 hover:translate-y-[-8px] border border-outline-variant/10 shadow-lg cursor-pointer"
                    >
